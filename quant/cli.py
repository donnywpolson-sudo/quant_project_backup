...
def check_memory_safety():
    try:
        ...
        if mem > config.RAM_CAP_BYTES:
            raise MemoryError(
                f'RSS {mem / 1024 ** 3:.2f}GB > cap {config.RAM_CAP_BYTES / 1024 ** 3:.2f}GB'
            )
    except ImportError:
        pass
...
def prune_features_by_manifest(df, manifest_path, target_col):
    ...
    essential = {
        ...
        'close', 'volume', 'session_id', 'regime'
    }

    non_feature = [
        c for c in df.columns
        if not c.startswith((
            'feature_', 'ratio_', 'pair_', 'zscore',
            'cross_', 'htf_', '1h_', 'daily_'
