...
def simulate_execution_classification(df: pl.DataFrame) -> pl.DataFrame:
    ...
    unit_cost = (config.COMMISSION_PER_TRADE + config.SLIPPAGE_K * pl.col('spread') + config.VOL_PENALTY * pl.col('vol')).clip(0.0, 0.01)
    open_next = pl.col('open').shift(-1)
    close_next = pl.col('close').shift(-1)
    ret_exec = ((close_next - open_next) / open_next.clip(config.EPS, None)).fill_null(0.0)
    ret_exec = ret_exec.clip(-0.02, 0.02)
    df = df.with_columns([unit_cost.alias('cost'), ret_exec.alias('ret_exec')])
    pnl = (pl.col('target_exec') * pl.col('ret_exec') - pl.col('cost')).fill_nan(0.0).clip(-0.05, 0.05)
